...
//caution--
//this is probably not efficient and but it was fun to make it work.....also my noob documentation
...
int main()
{
   ...
   scanf("%d:%d",&hour2,&minute2);
    // calculated the minutes between the times and added it to the starting time....
 if(hour1==hour2){
   ...
   if(minute1>minute2)
     {
        minutes=((minute2+60)-minute1+60*(hour2-hour1-1))/2; //if staring times minute count is bigger than the
                                                             //other one then add 60 minutes to the hour2 and minus it from minute1....also 
                                                              //counting the hours in between
      ...
      if(minute1==minute2)
      {
          minutes=((hour2-hour1)*60)/2;  //in similarity this is when both minutes are equal
                                         //now i can merge them making it way more easy to write but this is fine tooo.....
      total=minute1+minutes;
     while((total)>=60)
        {
          total=total-60;
